...
class CircleButton:
    def __init__(self, center, radius, circle_color,hover_color, text="", font=None, text_color=(0,0,0),hover_text_cover=(255,255,255), border_color=None, border_width=0, action=None):
        self.center = center
        self.radius = radius
    ...
    def draw(self, surface):
        mouse_pos = pygame.mouse.get_pos()
# mouse_pos = pygame.mouse.get_pos()
is_hovered = (mouse_pos[0] - circle_center[0])**2 + (mouse_pos[1] - circle_center[1])**2 <= radius**2
        # Draw the circle
        ...
        # Draw the text
        if self.text and self.font:
            text_surf = self.font.render(self.text, True, self.hover_text_cover if is_hovered else self.text_color)
